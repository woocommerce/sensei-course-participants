...
/**
 * Sensei Course Participants Extension Dependencies Check
 *
 * @since 2.0.0
 */
class Sensei_Course_Participants_Dependency_Checker {
	...
	public static function add_php_notice() {
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'sensei-course-participants' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'sensei-course-participants' )
		);
		echo '</p></div>';
	}
	...
	public static function add_sensei_notice() {
		...
		$message = sprintf( __( '<strong>Sensei Course Participants</strong> requires that the plugin <strong>Sensei</strong> (minimum version: <strong>%1$s</strong>) is installed and activated.', 'sensei-course-participants' ), self::MINIMUM_SENSEI_VERSION );
		echo '<div class="error"><p>';
		echo wp_kses( $message, array( 'strong' => array() ) );
		echo '</p></div>';
	}
